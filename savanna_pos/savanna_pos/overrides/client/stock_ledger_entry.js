        ...
        if (!frm.doc.custom_slade_id) {
          frm.add_custom_button(
            ...
            function () {
              frappe.call({
                method:
                  "savanna_pos.savanna_pos.overrides.server.stock_ledger_entry.save_ledger_details",
                args: {
                  name: frm.doc.name,
                },
                callback: (response) => {
            ...
            if (frm.doc.custom_inventory_submitted_successfully) {
              frm.add_custom_button(
                ...
                function () {
                  frappe.call({
                    method:
                      "savanna_pos.savanna_pos.overrides.server.stock_ledger_entry.submit_stock_mvt_transition",
                    args: {
                      name: frm.doc.name,
                    },
                    callback: (response) => {
              ...
              frm.add_custom_button(
                ...
                function () {
                  frappe.call({
                    method:
                      "savanna_pos.savanna_pos.overrides.server.stock_ledger_entry.submit_stock_mvt_items",
                    args: {
                      name: frm.doc.name,
                    },
                    callback: (response) => {
          ...
          frm.add_custom_button(
            ...
            function () {
              frappe.call({
                method:
                  "savanna_pos.savanna_pos.overrides.server.stock_ledger_entry.fetch_stock_mvt",
                args: {
                  name: frm.doc.name,
                },
                callback: (response) => {
