...
def item_registration_on_success(response: dict, document_name: str, settings_name: str, **kwargs) -> None:
    ...
    if item.is_stock_item:
        frappe.enqueue(
            "savanna_pos.savanna_pos.apis.apis.submit_inventory",
            name=document_name,
            queue="long",
        )
...
def sales_information_submission_on_success(response: dict, document_name: str, doctype: str, settings_name: str, **kwargs) -> None:
    ...
    frappe.enqueue(
        "savanna_pos.savanna_pos.apis.apis.get_invoice_details",
        document_name=document_name,
...
#     )
#     frappe.enqueue(
#         "savanna_pos.savanna_pos.apis.remote_response_status_handlers.process_invoice_items",
#         document_name=document_name,
#         doctype=doctype,
...
def process_sales_transition(
    ...
    def handle_transition_success(response: dict, document_name: str, **kwargs) -> None:
        ...
        frappe.enqueue(
            "savanna_pos.savanna_pos.apis.remote_response_status_handlers.process_sales_sign",
            document_name=document_name,
            doctype=doctype,
            invoice_slade_id=response.get("id"),
            queue="long",
...
def purchase_search_on_success(response: dict, **kwargs) -> None:
    ...
    for sale in sales_list:
        ...
        frappe.enqueue(
            "savanna_pos.savanna_pos.apis.remote_response_status_handlers.fetch_purchase_items",
